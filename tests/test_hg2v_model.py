...
class ModelsToEmbedding(unittest.TestCase):
  ...
  def test_boolean_model_to_emb(self):
    ...
    emb = KerasModelToEmbedding(hypergraph, model)

    self.assertEqual(emb.dim, dimension)
    for node_idx in hypergraph.node:
      self.assertTrue(node_idx in emb.node)
      self.assertEqual(len(emb.node[node_idx].values), dimension)
    for edge_idx in hypergraph.edge:
      self.assertTrue(edge_idx in emb.edge)
      self.assertEqual(len(emb.edge[edge_idx].values), dimension)
